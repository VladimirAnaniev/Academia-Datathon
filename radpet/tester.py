...
def predict_for_coin(coin):
    ...
    score_mape = None
    for subdir, dirs, files in os.walk('./baseline/2features/' + str(coin) + '/checkpoints'):
        files.sort()
        ...
        df_y_pred.to_csv(os.path.join('./baseline/2features/' + str(coin), 'y_pred_test.csv'), index=False)
